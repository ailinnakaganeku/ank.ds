...
const boxes = (n: number) =>
  Array.from({ length: n }).map((_, i) => <Box key={i} n={i + 1} />);
...
export const FillVsFit: Story = {
  render: () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      <div>
        <div style={{ fontFamily: 'var(--ank-mono)', fontSize: 12, color: 'var(--ank-gray-700)', marginBottom: 8 }}>
          flow="fit" — empty tracks collapse, items grow to fill the row.
        </div>
        <AutoGrid flow="fit" min="200px" gap="4">{boxes(3)}</AutoGrid>
      </div>
      <div>
        <div style={{ fontFamily: 'var(--ank-mono)', fontSize: 12, color: 'var(--ank-gray-700)', marginBottom: 8 }}>
          flow="fill" — empty tracks remain, useful for stable column count.
        </div>
        <AutoGrid flow="fill" min="200px" gap="4">{boxes(3)}</AutoGrid>
      </div>
    </div>
  ),
};
